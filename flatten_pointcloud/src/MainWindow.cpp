...
{
	mpGroundMeshCtrl = new wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(500, -1), wxTE_READONLY);

	mpGroundMeshFileButton = new wxButton(this, wxID_ANY, "Browse");
	mpGroundMeshFileButton->Bind(wxEVT_BUTTON, &cMainWindow::OnGroundMeshFile, this);
	...
	mpFlattenButton = new wxButton(this, wxID_ANY, "Flatten Point Cloud");
	mpFlattenButton->Disable();
	mpFlattenButton->Bind(wxEVT_BUTTON, &cMainWindow::OnFlatten, this);

	mpProgressCtrl = new cFileProgressCtrl(this, wxID_ANY, "Pass", 200);
...
void cMainWindow::CreateLayout()
{
	...
	auto* gm_sizer = new wxBoxSizer(wxHORIZONTAL);

	gm_sizer->Add(new wxStaticText(this, wxID_ANY, "Ground Mesh File: "), 0, wxALIGN_CENTER_VERTICAL);
	gm_sizer->AddSpacer(5);
	gm_sizer->Add(mpGroundMeshCtrl, 1, wxEXPAND);
	gm_sizer->AddSpacer(5);
	gm_sizer->Add(mpGroundMeshFileButton, 0, wxALIGN_CENTER_VERTICAL);
	gm_sizer->AddSpacer(5);
	topsizer->Add(gm_sizer, wxSizerFlags().Proportion(0).Expand());
	...
	topsizer->Add(mpFlattenButton, wxSizerFlags().Proportion(0).Expand());
...
// event handlers
void cMainWindow::OnGroundMeshFile(wxCommandEvent& WXUNUSED(event))
{
	wxFileDialog dlg(this, _("Open file"), "", "",
		"Ground Mesh (*.csv)|*.csv", wxFD_OPEN | wxFD_FILE_MUST_EXIST);

	if (dlg.ShowModal() == wxID_CANCEL)
		return;     // the user changed their mind...

	mGroundMesh = dlg.GetPath().ToStdString();
	mpGroundMeshCtrl->SetValue(mGroundMesh);

	if ((!mpSrcCtrl->GetValue().IsEmpty()) && (!mpDstCtrl->GetValue().IsEmpty()))
		mpFlattenButton->Enable();
}
...
void cMainWindow::OnSourceFile(wxCommandEvent& WXUNUSED(event))
{
	...
	if ((!mpGroundMeshCtrl->GetValue().IsEmpty()) && (!mpDstCtrl->GetValue().IsEmpty()))
		mpFlattenButton->Enable();
}
...
void cMainWindow::OnDestinationDirectory(wxCommandEvent& WXUNUSED(event))
{
	...
	if ((!mpGroundMeshCtrl->GetValue().IsEmpty()) && (!mpSrcCtrl->GetValue().IsEmpty()))
		mpFlattenButton->Enable();
}

void cMainWindow::OnFlatten(wxCommandEvent& WXUNUSED(event))
{
	using namespace nStringUtils;

	if (!load_ground_data(mGroundMesh.ToStdString()))
	{
		std::cerr << "Error: " << mGroundMesh << " could not be loaded." << std::endl;
		return;
	}
