...
void cMainWindow::CreateControls()
{
	...
	mpIronbow = new wxRadioButton(this, wxID_ANY, "Ironbow", wxDefaultPosition, wxDefaultSize, wxRB_GROUP);
...
void cMainWindow::CreateLayout()
{
	...
	auto* color_sz = new wxStaticBoxSizer(wxHORIZONTAL, this, "Color Pallet");
	color_sz->Add(mpIronbow, wxSizerFlags().Proportion(0).Expand());
	color_sz->AddSpacer(10);
	color_sz->Add(mpRainbow, wxSizerFlags().Proportion(0).Expand());
	color_sz->AddSpacer(10);
	color_sz->Add(mpRainbowHC, wxSizerFlags().Proportion(0).Expand());
	color_sz->AddSpacer(10);
