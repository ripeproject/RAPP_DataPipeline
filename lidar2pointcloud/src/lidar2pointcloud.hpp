...
public:
	...
	/**
	 * Set the "output options" for writing out point cloud data.
	 *
	 * AGGREGATE:				An aggregate point cloud is where all of the individual
	 *							point clouds generated by the individual lidar scan are
	 *							combined into one point cloud.
	 * REDUCED_SINGLE_FRAMES:	The point cloud generated by the individual lidar scan is
	 *							stored with all points outside of some bounds are removed.
	 * SENSOR_SINGLE_FRAMES:	The point cloud generated by the individual lidar scan is
	 ...
	 *
	 * BASIC:		Save only the point cloud data
	 * FRAME_ID:	Save point cloud with frame id information
	...
	const cKinematics* getKinematicModel() const;
