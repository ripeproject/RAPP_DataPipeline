...
namespace
{
    uint32_t to_uint32(const v1::cBlockID& blockID)
    {
...
uint32_t v1::crc(const v1::cBlockID& blockID, const std::byte* buf, std::size_t len)
{
...
uint32_t v1::crc(const v1::cBlockID& blockID)
{
    uint32_t id = to_uint32(blockID);
    auto c = update_crc(0xffffffffUL, reinterpret_cast<const std::byte*>(&id), sizeof(id));
    auto dataID = blockID.dataID();
