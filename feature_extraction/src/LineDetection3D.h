...
{
public:
	LineDetection3D() = default;
	~LineDetection3D() = default;

	void run(PointCloud<double> &data, int k, std::vector<PLANE>& planes, 
	...
	bool maskFromPoint(std::vector<cv::Point2d>& pts2d, double radius, double& xmin, double& ymin,
						double& xmax, double& ymax, int& margin, cv::Mat& mask);

	void lineFromMask(cv::Mat& mask, int thLineLengthPixel, 
...
public:
	...
	double mScale = 1.0;
	double mMagnitd = 0.0;
	std::vector<PCAInfo> mPcaInfos;
	PointCloud<double> mPointData;
};
