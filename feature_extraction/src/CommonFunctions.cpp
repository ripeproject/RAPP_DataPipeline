...
{
	void lineFitting(int rows, int cols, std::vector<cv::Point>& contour, double thMinimalLineLength,
					std::vector<std::vector<cv::Point2d>>& lines)
	{
		...
		subDivision(straightString, contour, 0, contour.size() - 1,
			minDeviation, int(thMinimalLineLength));
	...
	void subDivision(std::vector<std::vector<cv::Point> >& straightString, std::vector<cv::Point>& contour,
...
void PCAFunctions::MCMD_OutlierRemoval( std::vector<std::vector<double> > &pointData, PCAInfo &pcaInfo )
{
	...
	double median_OD = median( sorted_ODs );
	...
	double MAD_OD = a * median( abs_diff_ODs ) + 1e-6;
...
double PCAFunctions::median(std::vector<double> dataset)
{
